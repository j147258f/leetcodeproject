...
public class StringToInteger_atoi {
    ...
    public static int myAtoi(String str) {
        ...
        Integer[] stack = new Integer[chars.length];
        int stackIndex = 0;
        ...
        for (int i = 0; i < chars.length; i++) {
            if (chars[i] == '-' || chars[i] == '+') {
                ...
                if (chars[i] == '-') positive = false;
                if (chars[i] == '+') positive = true;
            } else if (chars[i] == ' ') {
                if (stackIndex != 0) break;
                if(positive!=null) break;
                continue;
            } else if (chars[i] >= '0' && chars[i] <= '9') {
                stack[stackIndex] = chars[i] - 48;
                stackIndex++;
        ...
        for (int i1 = 0; i1 < stack.length; i1++) {
            ...
            if (positive) {
                if ((result * 10) % 10 != 0 || result * 10 + stack[i1] < result) {
